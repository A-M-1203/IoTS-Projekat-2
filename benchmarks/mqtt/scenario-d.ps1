...
$outputFile = Join-Path $resultsDir "scenario-d_qos${Qos}_${timestamp}.txt"

Write-Host "=== Scenario D: Real-Time Alerting (MQTT) ==="

Write-Host "Restarting ingestion with INJECT_CRITICAL=true..."
docker compose --profile mqtt stop data-ingestion-service 2>$null
docker compose --profile mqtt rm -f data-ingestion-service 2>$null
...
docker compose --profile mqtt up -d data-ingestion-service
Start-Sleep -Seconds 15

Write-Host "Collecting analytics logs for $DurationSeconds seconds..."
$logStart = Get-Date
...
$logs | Out-File -FilePath (Join-Path $resultsDir "scenario-d_logs_${timestamp}.txt") -Encoding utf8
